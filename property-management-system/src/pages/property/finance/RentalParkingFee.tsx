...
import {
  getRentalParkingFees,
  generateRentalParkingFees,
  payRentalParkingFee,
  type RentalParkingFeeRecord,
  type RentalFeeQueryParams,
...
const RentalParkingFee: React.FC<Props> = ({ projectId, onStatsChange }) => {
  const [data, setData] = useState<RentalParkingFeeRecord[]>([]);
  ...
  const loadData = useCallback(async (p: number) => {
    setLoading(true);
    try {
      const params: RentalFeeQueryParams = { projectId, page: p, pageSize: 10, period };
      if (keyword) params.keyword = keyword;
      if (status) params.status = status;
      const result = await getRentalParkingFees(params);
      setData(result.list);
  ...
  const handleGenerate = async () => {
    setGenerating(true);
    try {
      const result = await generateRentalParkingFees(projectId, period);
      if (result.length === 0) {
        message.info('该账期已存在账单或没有活跃订阅');
      } else {
        message.success(`成功生成 ${result.length} 条月租账单`);
      }
      loadData(1);
      onStatsChange();
    } catch (err: any) {
  ...
  const handlePay = async (id: number) => {
    try {
      await payRentalParkingFee(id, 'wechat');
  ...
  const columns: ColumnsType<RentalParkingFeeRecord> = [
...
export default RentalParkingFee;
